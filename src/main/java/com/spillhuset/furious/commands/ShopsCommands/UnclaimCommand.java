...
public class UnclaimCommand implements SubCommandInterface {
    ...
    public UnclaimCommand(Furious plugin) { this.plugin = plugin; }
    ...
    public boolean execute(CommandSender sender, String[] args) {
        if (args.length < 2) {
            Components.sendErrorMessage(sender, "Usage: /shops unclaim <shopName>");
            return true;
        }
        plugin.shopsService.unclaimShop(sender, args[1]);
        return true;
    }
    ...
    @Override
    public String getPermission() { return "furious.shops.unclaim"; }
}
