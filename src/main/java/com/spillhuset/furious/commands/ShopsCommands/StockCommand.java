...
public class StockCommand implements SubCommandInterface {
    private final Furious plugin;
    public StockCommand(Furious plugin) { this.plugin = plugin; }
    ...
    public boolean execute(CommandSender sender, String[] args) {
        if (args.length < 4) {
            Components.sendInfoMessage(sender, "Usage: /shops stock <shop> <material> <amount>");
            return true;
        }
        String shop = args[1];
        String mat = args[2];
        ...
        plugin.shopsService.stock(sender, shop, mat, amount);
        return true;
    }
    ...
    @Override
    public String getPermission() { return "furious.shops.stock"; }
}
