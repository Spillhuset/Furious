...
public class SpawnCommand implements SubCommandInterface {
    ...
    public boolean execute(CommandSender sender, String[] args) {
        if (!(sender instanceof Player player)) {
            Components.sendErrorMessage(sender, "Only players can set spawn.");
            return true;
        }
        if (args.length < 2) {
            Components.sendInfoMessage(sender, "Usage: /shops spawn <name>");
            return true;
        }
        plugin.shopsService.setSpawn(sender, args[1], player.getLocation());
        return true;
    }
