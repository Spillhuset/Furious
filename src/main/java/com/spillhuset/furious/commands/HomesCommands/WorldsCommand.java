...
public class WorldsCommand implements SubCommandInterface {
    ...
    public List<String> tabComplete(CommandSender sender, String[] args) {
        // This subcommand is invoked when args[0] == "worlds".
        // Completions:
        // - args.length == 1 -> suggest world names (typing first argument after 'worlds')
        // - args.length == 2 -> suggest matching world names by prefix
        // - args.length == 3 -> suggest enable/disable filtered by prefix
        String prefix = args.length >= 2 ? args[1].toLowerCase(Locale.ROOT) : "";
        if (args.length == 1) {
            // After typing 'homes worlds ', suggest all worlds
    ...
    public boolean execute(CommandSender sender, String[] args) {
        ...
        if (args.length == 1) {
            plugin.homesService.listWorlds(sender);
            Components.sendInfoMessage(sender, "Usage: /homes worlds <world> <enable|disable>");
            return true;
        }
        if (args.length == 3) {
            ...
            plugin.homesService.setWorldEnabled(sender, world.getUID(), enable);
            return true;
        }
