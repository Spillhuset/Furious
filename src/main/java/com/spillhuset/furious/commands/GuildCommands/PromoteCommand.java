...
public class PromoteCommand implements SubCommandInterface {
    private final Furious plugin;

    public PromoteCommand(Furious plugin) {
    ...
    public List<String> tabComplete(CommandSender sender, String[] args) {
        ...
        // Admin: /guild promote <guildName> <player>
        if (canOthers) {
            if (args.length == 2) {
            ...
            if (args.length == 3) {
                ...
                for (Map.Entry<UUID, GuildRole> entry : guild.getMembers().entrySet()) {
                    ...
                    if (role == GuildRole.ADMIN) continue; // already max
        ...
        if (sender instanceof Player player) {
            if (args.length == 2 && can(sender, false)) {
                ...
                for (Map.Entry<UUID, GuildRole> entry : guild.getMembers().entrySet()) {
                    ...
                    GuildRole role = entry.getValue();
                    if (role == GuildRole.ADMIN) continue; // cannot promote admin
                    OfflinePlayer p = Bukkit.getOfflinePlayer(member);
                    String name = p.getName();
                    if (name != null && name.toLowerCase().startsWith(prefix)) {
                        if (!completions.contains(name)) completions.add(name);
    ...
    public boolean execute(CommandSender sender, String[] args) {
        // Admin form: /guild promote <guildName> <player>
        if (args.length >= 3 && can(sender, true, true)) {
            ...
            GuildService.RoleChangeResult res = plugin.guildService.promoteByName(guildName, target.getUniqueId());
            switch (res) {
                case SUCCESS -> Components.sendSuccessMessage(sender, "Promoted " + target.getName() + " in guild " + guildName + ".");
                ...
                case TARGET_ALREADY_AT_MAX -> Components.sendErrorMessage(sender, "That player is already at the highest rank.");
                default -> Components.sendErrorMessage(sender, "Failed to promote player.");
            }
            return true;
        }

        // Player form: /guild promote <player>
        if (args.length >= 2 && can(sender, true, false)) {
            if (!(sender instanceof Player player)) {
                Components.sendErrorMessage(sender, "Only players can use this form of /guild promote.");
                return true;
            }
            ...
            GuildService.RoleChangeResult res = plugin.guildService.promote(player.getUniqueId(), target.getUniqueId());
            switch (res) {
                case SUCCESS -> Components.sendSuccessMessage(sender, "Promoted " + target.getName() + " in your guild.");
                case ACTOR_NOT_IN_GUILD -> Components.sendErrorMessage(sender, "You are not in a guild.");
                case ACTOR_NOT_ADMIN -> Components.sendErrorMessage(sender, "You must be an admin/owner to promote members.");
        ...
        Components.sendInfoMessage(sender, "Usage: /guild promote " + (can(sender, false, true) ? "[guild] " : "") + "<player>");
        return true;
    }
