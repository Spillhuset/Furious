...
public class TamingCommand implements SubCommandInterface {
    ...
    public TamingCommand(Furious plugin) {
    ...
    public boolean execute(CommandSender sender, String[] args) {
        if (args.length >= 2 && args[1].equalsIgnoreCase("clear")) {
            ...
            switch (scope) {
                case "month" -> {
                    plugin.tamingService.clearMonthFirst();
                    Components.sendSuccessMessage(sender, "Cleared this-month-first winners.");
                    return true;
                }
                case "year" -> {
                    plugin.tamingService.clearYearFirst();
                    Components.sendSuccessMessage(sender, "Cleared this-year-first winners.");
                    return true;
                }
                case "all" -> {
    ...
    private void showChecklist(CommandSender sender, UUID playerId, String displayName) {
        ...
        for (EntityType t : all) {
            ...
            String simple;
            try {
                key = t.getKey().asString().toLowerCase(Locale.ROOT);
                simple = t.getKey().value().toLowerCase(Locale.ROOT);
            } catch (Throwable e) {
                key = t.toString().toLowerCase(Locale.ROOT);
                simple = key;
            }
    ...
    public String getPermission() {
        return "furious.checklist.taming";
    }
}
