...
/**
 * Subcommand for adding an item to a shop.
 */
public class AddSubCommand implements SubCommand {
    ...
    public boolean execute(@NotNull CommandSender sender, @NotNull String[] args) {
        ...
        if (shopsManager.addItemToShop(shopName, material)) {
            sender.sendMessage("§aAdded §e" + material.name() + "§a to shop §e" + shopName + "§a!");
            return true;
        } else {
            sender.sendMessage("§cFailed to add item! It may already be in the shop.");
            return false;
        }
    }
