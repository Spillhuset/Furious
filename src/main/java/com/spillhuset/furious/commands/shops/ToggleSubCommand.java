...
/**
 * Subcommand for toggling whether an item can be bought from or sold to a shop.
 */
public class ToggleSubCommand implements SubCommand {
    ...
    /**
     * Constructor for ToggleSubCommand.
     *
     * @param plugin The main plugin instance
     */
    public ToggleSubCommand(Furious plugin) {
        this.plugin = plugin;
    ...
    public String getDescription() {
        return "Toggle whether an item can be bought from or sold to the shop";
    }
    ...
    public boolean execute(@NotNull CommandSender sender, @NotNull String[] args) {
        ...
        if (!toggleType.equals("buy") && !toggleType.equals("sell")) {
            sender.sendMessage("§cInvalid toggle type. Use 'buy' or 'sell'.");
            getUsage(sender);
            return false;
        }
        ...
        // Toggle buying or selling based on the toggle type
        if (toggleType.equals("buy")) {
            newState = shopsManager.toggleBuyItem(shopName, material);
        ...
        } else { // toggleType.equals("sell")
            newState = shopsManager.toggleSellItem(shopName, material);
    ...
    public List<String> tabComplete(CommandSender sender, @NotNull String[] args) {
        if (args.length == 1) {
            // Suggest "buy" or "sell" as the first argument
